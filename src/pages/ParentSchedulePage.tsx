...
import { useParentChildrenSchedule } from '@/hooks/useParentSchedule';
import type {
  ChildSessionWithInfo,
  ParentScheduleDay,
} from '@/interface/parent-schedule.interface';
import type { ScheduleSession } from '@/interface/student-schedule.interface';
import {
  ArrowLeft,
  CalendarDays,
  ChevronLeft,
...
const ParentSchedulePage = () => {
  ...
  const { data: parentData } = useParent(parentId || '');
  ...
  const weeklyData = scheduleData?.combinedSchedule;
  ...
  const periodGridData = useMemo(() => {
    ...
    weeklyData.days.forEach((day: ParentScheduleDay) => {
      ...
      day.sessions.forEach((session: ChildSessionWithInfo) => {
      ...
      <div className="mx-auto max-w-[1600px] p-6 space-y-6">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-sm font-medium text-blue-600">Lịch học các con</p>
            <h1 className="text-2xl font-bold text-gray-900">
              Lịch học của {parentData?.data.displayName || 'phụ huynh'} ({scheduleData?.children?.length || 0} con)
            </h1>
