...
export default function EditAssignmentPage() {
  ...
  // Prefill form when assignment data loads
  useEffect(() => {
    if (existingAssignment) {
      // Check if assignment can be edited
      const editable = canEditAssignment(existingAssignment);
      setCanEdit(editable);
      ...
      const formValues: AssignmentFormValues = {
        ...
        timeLimit: existingAssignment.timeLimit || undefined,
        maxAttempts: existingAssignment.maxAttempts || 1,
        isPublished: existingAssignment.isPublished || false,
        activities: (existingAssignment.assignmentActivities || []).map((activity: any) => ({
          type: activity.type,
  ...
  const onSubmit: SubmitHandler<AssignmentFormValues> = async (data: any) => {
    ...
    if (!canEdit) {
      toast.error('Không thể chỉnh sửa bài tập sau khi đã đến giờ bắt đầu');
      return;
    }
    ...
    try {
      ...
      const timeErrors: { dueDate?: string } = {};
      if (data.dueDate) {
      ...
      const dateErrors: { dueDate?: string } = {};
      if (data.dueDate) {
        const dueDate = new Date(data.dueDate);
      ...
      await assignmentApi.updateAssignment(assignmentId, updateDto);
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      await downloadAssignmentPdf(assignmentId, watch('title'));
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">
                Update assignment details and activities
              </Typography>
            </Box>
            <Button
              variant="outlined"
            ...
            <Grid item xs={12} sx={{ mb: 3 }}>
              <Alert severity="error" sx={{ mb: 2 }}>
                <Typography variant="body1" fontWeight={600}>
                  Không thể chỉnh sửa bài tập sau khi đã đến giờ bắt đầu
                </Typography>
                <Typography variant="body2" sx={{ mt: 1 }}>
                  Bài tập này đã bắt đầu và không thể chỉnh sửa. Bạn có thể xem thông tin nhưng không thể thay đổi nội dung.
                </Typography>
              </Alert>
            </Grid>
          )}
                              ...
                              <Grid item xs={12} md={2}>
                                <FormControl fullWidth size="small">
                                  ...
                                  <Controller
                                    ...
                                    control={control}
                                    render={({ field }) => (
                                      <Select {...field} label="Type *" disabled={!canEdit}>
                                        {ACTIVITY_TYPES.map((type) => (
                              ...
                              <Grid item xs={12} md={3}>
                                <FormControl fullWidth size="small">
                                  ...
                                  <Controller
                                    ...
                                    control={control}
                                    render={({ field }) => (
                                      <Select {...field} label="Difficulty" disabled={!canEdit}>
              ...
              <Paper sx={{ p: 3 }}>
                <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
                  ...
                  <Button
                    ...
                    type="submit"
                    startIcon={<Save />}
                    disabled={!canEdit || isSubmitting || activityFields.length === 0}
                  >
