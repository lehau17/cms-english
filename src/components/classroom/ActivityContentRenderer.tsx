...
const ActivityContentRenderer: React.FC<ActivityContentRendererProps> = ({ type, content }) => {
  ...
  const renderMatchingContent = () => {
    // Handle both formats: pairs array OR leftItems/rightItems arrays
    ...
    if (content.pairs && Array.isArray(content.pairs)) {
      ...
      Array.isArray(content.leftItems) && Array.isArray(content.rightItems)) {
      // Format: { leftItems: [...], rightItems: [...] }
      pairs = content.leftItems.map((left: string, idx: number) => ({
        left: left || '',
    ...
    return (
      <div className="space-y-2">
        ...
        {pairs.length > 0 ? (
          ...
          <p className="text-sm text-gray-500 italic">Không có dữ liệu ghép nối</p>
        )}
      </div>
    );
  };
