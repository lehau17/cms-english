...
const StudentPage: React.FC = () => {
    // Mock data - tạo nhiều students để test phân trang
    ...
    const fetchStudents = async (
        ...
        // Trong thực tế sẽ gọi: GET /api/students?page=1&limit=5&search=john&status=active
        console.log(`API Call: /api/students?page=${page}&limit=${pageLimit}&search=${search}&status=${status}`);
        ...
        if (page === 1 && pageLimit === 5) {
            mockResponse = {
                ...
                data: {
                    ...
                    page: 1,
                    limit: 5,
                    totalItems: 15, // Giả sử server có tổng 15 students
                    totalPages: 3,
                    hasNextPage: true,
                    hasPrevPage: false
                }
        ...
        if (search) {
            const filtered = mockResponse.data.data.filter(student =>
        ...
        if (status !== 'all') {
            const filtered = mockResponse.data.data.filter(student => student.status === status);
            mockResponse.data.data = filtered;
            mockResponse.data.totalItems = filtered.length;
            mockResponse.data.totalPages = Math.ceil(filtered.length / pageLimit);
        }

        setStudents(mockResponse.data.data);
        setPagination({
    ...
    useEffect(() => {
        ...
        fetchStudents(page, pageLimit, search, status as 'all' | 'active' | 'inactive');
    }, []);
    ...
    const handlePageChange = (newPage: number): void => {
        ...
        fetchStudents(newPage, limit, searchTerm, statusFilter);
    };
    ...
    const handleLimitChange = (newLimit: number): void => {
        ...
        fetchStudents(1, newLimit, searchTerm, statusFilter);
    };
    ...
    const handleSearch = (search: string): void => {
        ...
        fetchStudents(1, limit, search, statusFilter);
    };
    ...
    const handleStatusFilter = (status: 'all' | 'active' | 'inactive'): void => {
        ...
        fetchStudents(1, limit, searchTerm, status);
    };
                                        ...
                                        <tr key={student.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center space-x-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="text-xs text-gray-400 italic">{student.bio}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="space-y-1">
                                                    ...
                                                    <div className="flex items-center space-x-2 text-sm">
                                                        ...
                                                        {student.phone && (student.phoneVerified ? (
                                                            <CheckCircle className="w-4 h-4 text-green-500" />
                                                        ) : (
                                                            <XCircle className="w-4 h-4 text-red-500" />
                                                        ))}
                                                    ...
                                                    <div className="flex items-center space-x-2 text-sm">
                                                        ...
                                                        <span className="text-gray-600">{formatDate(student.dob)}</span>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="space-y-2">
                                                    <span
                                                        className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${student.status === 'active'
                                                    ...
                                                    <div className="text-xs text-gray-500">
                                                        {student.gender} • {student.language.toUpperCase()}
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4">
                                                <span
                                                    className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${student.provider === 'google'
                                                            ? 'bg-red-100 text-red-800'
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="space-y-1 text-xs text-gray-500">
                                                    <div className="flex items-center space-x-2">
                                                        ...
                                                        <span>Login: {formatDateTime(student.lastLoginAt)}</span>
                                                    </div>
                                                    <div className="flex items-center space-x-2">
                                                        <Clock className="w-3 h-3" />
                                                        <span>Active: {formatDateTime(student.lastActiveAt)}</span>
                                                    </div>
                                                </div>
                                            </td>
                            ...
                            <div className="flex items-center">
                                ...
                                <div className="ml-4">
                                    ...
                                    <p className="text-2xl font-semibold text-gray-900">
                                        {mockData.data.data.filter((s: Student) => s.status === 'active').length}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-lg shadow-md p-6">
                            <div className="flex items-center">
                                ...
                                <div className="ml-4">
                                    ...
                                    <p className="text-2xl font-semibold text-gray-900">
                                        {mockData.data.data.filter((s: Student) => s.status === 'inactive').length}
                                    </p>
                                </div>
                            </div>
                        </div>
