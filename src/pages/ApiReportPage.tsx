...
interface ChatMessage {
  ...
  suggestions?: string[];
  executionSteps?: any[];
  charts?: any[]; // Chart configs for visualization (supports multiple charts)
  files?: Array<{
    filename: string;
    downloadUrl: string;
    recordCount?: number;
...
const ApiReportPage: React.FC = () => {
  ...
  const [streamingChart, setStreamingChart] = useState<any[]>([]); // Charts during streaming (supports multiple)
  ...
  const handleSendMessage = () => {
    ...
    setStreamingChart([]); // Clear previous charts
        ...
        const controller = await streamAgentChat(
          ...
          (chunk) => {
            if (chunk.type === 'metadata' && chunk.data?.conversationId) {
              ...
              setStreamingChart(prev => [...prev, chunk.chart]); // Append chart to array
          ...
          () => {
            ...
            setStreamingChart([]); // Clear streaming charts
            setIsStreaming(false);
            ...
            const newMessage: ChatMessage = {
              ...
              suggestions: [],
              executionSteps: [],
              charts: metadata.charts.length > 0 ? metadata.charts : undefined, // Save charts configs
                        ...
                        {!streamingResponse && isStreaming ? (
                          ...
                          <div className="space-y-3">
                            ...
                            {streamingChart.length > 0 && (
                              <div className="space-y-4">
                                {streamingChart.map((chart, index) => (
                      ...
                      <>
                        ...
                        {streamingChart.length > 0 && (
                          <div className="mt-4 space-y-4 animate-fadeIn">
                            {streamingChart.map((chart: any, idx: number) => (
                              <div key={idx}>
                                <ChartRenderer chart={chart} />
                              </div>
                            ))}
